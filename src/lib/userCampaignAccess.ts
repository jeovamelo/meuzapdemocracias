...
export async function getUserCampaignAccesses(userId: string): Promise<UserCampaignAccess[]> {
  ...
  const memberships = ((membershipResult.data || []) as MembershipRow[]).filter((membership) =>
    isApprovedMembership(membership.status),
  );
  ...
  memberCampaigns.forEach((campaign) => {
    ...
    if (membership)
      accesses.set(campaign.id, mapAccess(campaign, membership.role, membership.status));
  });
